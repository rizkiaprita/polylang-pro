...
abstract class PLL_REST_Translated_Object extends PLL_REST_Filtered_Object {
		...
		foreach ( $content_types as $type => $args ) {
			...
			if ( $args['translations'] ) {
				...
				register_rest_field(
					...
					'translations_slug',
					array(
						'get_callback'    => array( $this, 'get_translations_slug' ),
	 ...
	 */
	public function get_translations_slug( $object ) {
		$tmp = $this->model->{$this->type}->get_translations($object['id']);
		$tmp['id'] = get_post_field( 'post_name', $tmp['id']);
		$tmp['en'] = get_post_field( 'post_name', $tmp['en']);
		return $tmp;
	}
	/**
	 * Save translations
